...
void new_dynarec_init()
{
	...
	#if NEW_DYNAREC == NEW_DYNAREC_ARM
		if ((base_addr = dma_allocate_memory(1<<TARGET_SIZE_2)) <= 0)  
		{
			DebugMessage(M64MSG_ERROR, "mmap() failed 0x%X", base_addr);
		}
...
*/
  ...
  out=(u_char *)base_addr;
	DebugMessage(M64MSG_INFO, "new_dynarec_init() out %p, BASE_ADDR %p", (void*)out, (void*)BASE_ADDR);
